...
def create_router(database_exists: bool = False) -> APIRouter:
    ...
    if database_exists:
        try:
            ...
            logger.info(
                "Database-dependent endpoints (orders) registered successfully"
            )
        except Exception as e:
            logger.error(f"Failed to register database-dependent endpoints: {e}")
    else:
        logger.info(
            "Database instance not found - skipping orders and tables endpoints"
        )
        logger.info(
            "Create Lakebase resources using POST /api/v1/resources/create-lakebase-resources"
        )
